...
def current_user():
    ...
    conn = get_db()
    user = conn.execute("""
        SELECT id, username, email, display_name, theme, avatar_style, sound_enabled, stars
...
def record_game_play(uid,game,room_code):
    ...
    conn.commit();conn.close()
    return evaluate_medals(uid,game)
...
def registro():
    if request.method=="POST":
        ...
        cur=conn.execute("INSERT INTO usuarios(username,email,password_hash,display_name) VALUES(?,?,?,?)",(username,email,generate_password_hash(password),username))
        conn.commit(); session["user_id"]=cur.lastrowid; conn.close()
...
def login():
    ...
    conn=get_db(); user=conn.execute("SELECT * FROM usuarios WHERE username=?",(username,)).fetchone(); conn.close()
    if not user or not check_password_hash(user["password_hash"],password):
        flash("Usuario o contraseña incorrectos.","error"); return redirect(url_for("index"))
    session["user_id"]=user["id"]; return redirect(url_for("menu"))
...
def invite_to_room(data):
    ...
    if room.get("game")=="parchis_deluxe" and len(room.get("players",[]))>=4:
        emit("app_error",{"message":"La sala de Parchís ya tiene 4 jugadores."}); return
...
def accept_room_invitation(data):
    ...
    if room.get("game")=="parchis_deluxe" and len(room.get("players",[]))>=4 and not find_player(room,user["id"]):
        emit("app_error",{"message":"La sala de Parchís ya está completa."}); return
    if not find_player(room,user["id"]):
        if room.get("game")=="parchis_deluxe":
